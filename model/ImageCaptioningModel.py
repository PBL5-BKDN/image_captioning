            ...
            for input in batch:
                ...
                for token_id in input:
                    ...
                    if token in self.vocab.special_tokens:
                        emb = self.special_token_embedding(torch.tensor([self.vocab.w2i[token]])).squeeze(0)
                    else:
                        emb = torch.Tensor(self.ft.get_word_vector(token))
    ...
    def forward(self, image, caption_tokens):
       ...
       """
        ...
        with torch.no_grad():
            ...
            features = features[0]# (batch_size, 2048)
        # Encode ảnh
